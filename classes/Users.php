                        ...
                        class Manager extends ConnectionManager
                        {
                                  public function get()
                            {
                                ...
                                try {
                                    ...
                                    $sth->execute();
                                    $retval['r']['c'] = array(
             array('data' => 'id_user', 'title' => 'id_user'), array('data' => 'user', 'title' => 'user'), array('data' => 'name', 'title' => 'name'), array('data' => 'image', 'title' => 'image'), array('data' => 'password', 'title' => 'password'),
            ...
            array_push($retval['r']['d'], $row);
                               ...
                               public function set( $dt)
                        {
                            ...
                            try {
                                ...
                                $cnx =  $this->connectSqlSrv();
                                $sth =  $cnx->prepare('INSERT INTO tb_users(user,name,image,password)VALUES (:user,:name,:image,:password)');
                                $sth->execute( $dt);
                               ...
                               public function delete( $dt)
                        {
                            ...
                            try {
                                $cnx =  $this->connectSqlSrv();
                                $sth =  $cnx->prepare('DELETE FROM tb_users WHERE id_user=:id_user');
                                $sth->execute( $dt);
                                if ($retval['r'] = $sth->rowCount()) {
                                    $retval['data'] = true;
                                }
                               ...
                               public function update( $dt)
                        {
                            ...
                            try {
                                ...
                                $cnx =  $this->connectSqlSrv();
                                $sth =  $cnx->prepare('UPDATE tb_users SET user=:user,name=:name,image=:image,password=:password  WHERE id_user=:id_user');
                                $sth->execute( $dt);
                                if ($retval['r'] = $sth->rowCount()) {
                                    $retval['data'] = true;
                                }
