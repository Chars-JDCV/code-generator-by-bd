            ...
            array_push($retval['r']['d'], $row);
                               ...
                               public function set( $dt)
                        {
                            ...
                            try {
                                ...
                                $cnx =  $this->connectSqlSrv();
                                $sth =  $cnx->prepare('INSERT INTO tb_goods(id_sub_departament,short_good_name,full_good_name,good_description,good_brand,good_model,sibisep_serial_code,good_value,date,good_exists,quantity,is_unregistered)VALUES (:id_sub_departament,:short_good_name,:full_good_name,:good_description,:good_brand,:good_model,:sibisep_serial_code,:good_value,:date,:good_exists,:quantity,:is_unregistered)');
                                $sth->execute( $dt);
                               ...
                               public function delete( $dt)
                        {
                            ...
                            try {
                                $cnx =  $this->connectSqlSrv();
                                $sth =  $cnx->prepare('DELETE FROM tb_goods WHERE id_good=:id_good');
                                $sth->execute( $dt);
                                if ($retval['r'] = $sth->rowCount()) {
                                    $retval['data'] = true;
                                }
                               ...
                               public function update( $dt)
                        {
                            ...
                            try {
                                ...
                                $cnx =  $this->connectSqlSrv();
                                $sth =  $cnx->prepare('UPDATE tb_goods SET id_sub_departament=:id_sub_departament,short_good_name=:short_good_name,full_good_name=:full_good_name,good_description=:good_description,good_brand=:good_brand,good_model=:good_model,sibisep_serial_code=:sibisep_serial_code,good_value=:good_value,date=:date,good_exists=:good_exists,quantity=:quantity,is_unregistered=:is_unregistered  WHERE id_good=:id_good');
                                $sth->execute( $dt);
                                if ($retval['r'] = $sth->rowCount()) {
                                    $retval['data'] = true;
                                }
